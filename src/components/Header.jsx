...
export const Header = ({theme, setTheme, handleThemeSwitch}) => {
    ...
    return (
        <div className={`z-40 transition py-3 delay-100 ease-in-out w-full ${isScrolled ? "fixed bg-gray-300 dark:bg-black" : "sticky bg-white dark:bg-gray-900"}`} ref={headerRef}>
            <header className='flex justify-between items-center px-3 max-w-6xl mx-auto w-full z-50'>
                ...
                <div>
                    <ul className='flex mt-1 space-x-2 xs:space-x-4 sm:space-x-6 md:space-x-12  justify-center items-center'>
                        ...
                        <li className={`cursor-pointer py-2 xxs:text-xs xs:text-sm font-semibold border-b-[3px] flex items-center  ${
                            ...
                            Axtarış <FaSearch className="w-3 h-3 mt-1 ml-2" />
                        </li>
                        <li className={`cursor-pointer py-2 xxs:text-xs xs:text-sm font-semibold border-b-[3px] ${
                            (pathMatchRoute("/sign-in") || pathMatchRoute("/profile")) ? "border-b-red-500 text-black dark:text-white" : "text-gray-500 dark:text-gray-300 border-b-transparent"
                        }`} onClick={() => navigate("/profile")}>{pageState}</li>
                        <li className='cursor-pointer py-2 xxs:text-xs xs:text-sm font-semibold'>
